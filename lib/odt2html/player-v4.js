...
function bgplay(url) {
	...
	parent.ga('send', 'event', 'TD Sound', 'play', url);
}
...
function play_audio(url, title) {
	...
	parent.ga('send', 'event', 'Audio', 'play', url);
}
...
function play_video(url, title) {
	...
	parent.ga('send', 'event', 'Video', 'play', url);
}
...
/* Set the URL of the audio files for the player to play. The audio
   file extension should be omitted from the URL unless the file
   is available only in MP3 format. */
AudioPlayer.prototype.play_url = function(url) {
	if(url.substr(-4) === ".mp3") {		/* MP3 only */
		...
		this.source_ogg.src = url.replace("#",".ogg#");
		this.source_mp3.src = url.replace("#",".mp3#");
	} else {
